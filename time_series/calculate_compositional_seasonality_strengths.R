...
# Load list of candidate composition effect species
candidate_compositional_species <- read_tsv("./candidate_compositional_species.tsv")

# Load abundances, select only candidate composition sites, remove the
# candidate composition effect species, and renormalise remaining species
# abundances
abundances <- read_tsv("../abundances/species_abundances_decontaminated.tsv") %>%
  inner_join(candidate_compositional_species %>% select(site, location)) %>%
  select(sample, site, location, timepoint, species, abundance) %>%
  left_join(candidate_compositional_species %>% 
              select(site, location, compositional_species = species)) %>%
  filter(! species == compositional_species) %>%
  select(-compositional_species) %>%
...
# Write to file
write_tsv(seasonality_strengths, "compositional_seasonality_strengths.tsv")
